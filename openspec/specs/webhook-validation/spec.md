...
This capability defines how the webhook ingestion service validates incoming request payloads from external platforms (Shopify, Mindbody) before any processing or persistence occurs. Validation is enforced at the API boundary using Zod schemas, ensuring only well-formed payloads proceed and all rejections return structured error responses.
