# Spec: Signal Normalization

## Purpose

Defines how webhook route handlers map source-specific payloads to a uniform `NormalizedInput` type, and how `normalizeSignals` extracts a typed `Signal[]` from that input without any source-specific branching logic.
