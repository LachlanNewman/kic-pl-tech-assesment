...
describe('resolveProfileMergeConflict', () => {
  it('picks the customer with the highest-confidence signal as winner', () => {
    const matches: CustomerSignalMatch[] = [
      {
        customerId: 'cust_a',
        matchedSignals: [makeSignal('device_id', 'dev_abc', 2)],
        confidence: 2,
      },
      {
        customerId: 'cust_b',
        matchedSignals: [makeSignal('email', 'b@example.com', 3)],
        confidence: 3,
      },
    ];
  ...
  it('returns all other customers as losers when three or more match', () => {
    const matches: CustomerSignalMatch[] = [
      {
        customerId: 'cust_a',
        matchedSignals: [makeSignal('device_id', 'dev_1', 2)],
        confidence: 2,
      },
      {
        customerId: 'cust_b',
      ...
      {
        customerId: 'cust_c',
        matchedSignals: [makeSignal('phone', '+61400000002', 3)],
        confidence: 3,
      },
    ];
  ...
  it('uses first match as winner when signal confidences are equal', () => {
    const matches: CustomerSignalMatch[] = [
      {
        customerId: 'cust_a',
        matchedSignals: [makeSignal('email', 'a@example.com', 3)],
        confidence: 3,
      },
      {
        customerId: 'cust_b',
        matchedSignals: [makeSignal('phone', '+61400000001', 3)],
        confidence: 3,
      },
    ];
