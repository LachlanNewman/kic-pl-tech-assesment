...
export async function identityResolution(payload: WebhookPayload): Promise<string> {
  ...
  try {
    ...
    logger.debug(
      ...
      'identityResolution: grouped matches',
    );

    const matchesAboveThreshold = getMatchesAboveThreshold(groupedMatches);
    logger.debug(
      { matchesAboveThresholdCount: matchesAboveThreshold.length },
      'identityResolution: matches above confidence threshold',
    );
    ...
    logger.error({ error }, 'identityResolution: error resolving identity resolution');
    throw HttpError.fromError(error, errors.unknownError);
  }
}
